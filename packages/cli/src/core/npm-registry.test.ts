...
import { NetworkError, TimeoutError, ValidationError } from "@metabase/client/errors";
...
const CALLER_USER_AGENT = "metabase-cli/9.9.9";
...
describe("fetchNpmDistTags", () => {
  it("hits /-/package/<scope%2Fname>/dist-tags on the default registry", async () => {
    ...
    const result = await fetchNpmDistTags("@metabase/cli", {
      fetchImpl,
      userAgent: CALLER_USER_AGENT,
    });
  ...
  it("URL-encodes the scope separator only (keeps @ literal)", async () => {
    ...
    await fetchNpmDistTags("unscoped-pkg", { fetchImpl, userAgent: CALLER_USER_AGENT });
  ...
  it("throws HttpError for a 404 from the registry", async () => {
    ...
    let captured: unknown;
    try {
      await fetchNpmDistTags("@no/such-thing", { fetchImpl, userAgent: CALLER_USER_AGENT });
    } catch (error) {
      captured = error;
    }
  ...
  it("throws ValidationError when the registry omits `latest`", async () => {
    ...
    await expect(
      fetchNpmDistTags("@metabase/cli", { fetchImpl, userAgent: CALLER_USER_AGENT }),
