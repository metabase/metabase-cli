...
} from "@metabase/client/auth/credential";
...
async function ensureFreshCredential(
  ...
  try {
    refreshed = await refreshOAuthCredential(issuerUrl, credential, Date.now(), USER_AGENT);
  } catch {
...
async function refreshAndPersist(
  ...
  const refreshed = await refreshOAuthCredential(issuerUrl, credential, Date.now(), USER_AGENT);
  await persistRefreshed(profile, issuerUrl, refreshed);
  return refreshed;
}
