...
import { ConfigError, isNotFoundError, ValidationError } from "../errors";
...
export function profilesFilePath(): string {
  return join(configDir(), profilesFileName(resolveProfileStore()));
}
...
// Every secret this module reads or writes belongs to the profile store the process runs under, so
// two stores on one machine (a human's `mb` and an agent's) can neither read nor clear each other's.
function currentKeyringService(): string {
  return keyringService(resolveProfileStore());
}
...
function trySetKeyring(key: CredentialAccount, value: string): boolean {
  ...
  try {
    new Entry(currentKeyringService(), key).setPassword(value);
    return true;
  } catch {
    return false;
  }
...
function tryReadKeyring(key: CredentialAccount): string | null | undefined {
  ...
  try {
    return new Entry(currentKeyringService(), key).getPassword();
  } catch {
    return undefined;
  }
}
...
function removeKeyringEntry(key: CredentialAccount): KeyringRemoval {
  ...
  try {
    return new Entry(currentKeyringService(), key).deletePassword() ? "removed" : "absent";
  } catch {
    return "failed";
  }
}
...
// The legacy files predate profile stores and belong to the default one. Another store must not
// warn about them, and — since a write cleans them up — must not delete them either.
function ownsLegacyArtifacts(): boolean {
  return resolveProfileStore() === DEFAULT_PROFILE_STORE;
}

async function detectLegacyArtifacts(): Promise<void> {
  if (!ownsLegacyArtifacts()) {
    return;
  }
...
function persistSecret(key: CredentialAccount, value: string): CredentialLocation {
  if (trySetKeyring(key, value)) {
    return { backend: "keyring", service: currentKeyringService(), account: key };
  }
  ...
  if (keyringRequired(store)) {
    throw new ConfigError(keyringRequiredMessage(store));
  }
  return fileLocation(key);
}

function keyringRequiredMessage(store: string): string {
  const cause =
    readEnv(ENV_DISABLE_KEYRING) === "1"
      ? `the OS keychain is disabled via ${ENV_DISABLE_KEYRING}`
      : "the OS keychain is unavailable";
  return (
    `Refusing to store a credential outside the OS keychain: ${cause}, and the "${store}" profile store keeps secrets nowhere else. ` +
    "Nothing was written. Supply the credential in the environment instead (MB_URL/MB_API_KEY)."
  );
}
...
export async function writeOAuthProfile(
  ...
  return onFile
    ...
    : { backend: "keyring", service: currentKeyringService(), account: accessKey };
}
