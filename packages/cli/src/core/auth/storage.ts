...
type ProfileApiKeyAccount = `profile:${string}:apiKey`;
type ProfileOAuthAccessAccount = `profile:${string}:oauthAccess`;
type ProfileOAuthRefreshAccount = `profile:${string}:oauthRefresh`;
...
interface KeyringLocation {
  backend: "keyring";
  service: string;
  account: CredentialAccount;
}

type KeyringFallbackReason = "disabled" | "unavailable";
