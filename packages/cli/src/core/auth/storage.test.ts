...
describe("profile stores", () => {
  ...
  it("keeps the default store's file and keychain service exactly where they were", async () => {
    const location = await writeProfile({ url: "https://mb.example.com", apiKey: "key-cli" });
    ...
    expect(profilesFilePath()).toBe(join(configDir(), "profiles.json"));
  });

  it("gives another store its own file and its own keychain service", async () => {
    process.env["MB_PROFILE_STORE"] = "agent";

    const location = await writeProfile({ url: "https://mb.example.com", apiKey: "key-agent" });

    expect(location).toEqual({
      backend: "keyring",
      service: "metabase-agent",
      account: "profile:default:apiKey",
    });
    expect(profilesFilePath()).toBe(join(configDir(), "profiles.agent.json"));
  });

  it("cannot read, overwrite or clear a same-named profile in another store", async () => {
    await writeProfile({ url: "https://human.example.com", apiKey: "key-cli" });

    process.env["MB_PROFILE_STORE"] = "agent";
    expect(await readProfileCredential()).toBeNull();
    await writeProfile({ url: "https://agent.example.com", apiKey: "key-agent" });
    expect(await clearProfile()).toBe(true);
    expect(await readProfileCredential()).toBeNull();

    delete process.env["MB_PROFILE_STORE"];
    expect(await readProfileCredential()).toEqual({
      url: "https://human.example.com",
      credential: { kind: "apiKey", apiKey: "key-cli" },
    });
  });

  it("rejects a store name that could escape the config directory", () => {
    process.env["MB_PROFILE_STORE"] = "../../etc";

    expect(() => profilesFilePath()).toThrowError(
      'MB_PROFILE_STORE must be a lowercase name like "agent" (letters, digits and dashes, starting with a letter), not "../../etc".',
    );
  });
});

describe("stores that keep secrets in the keychain only", () => {
  let home: TempConfigHome;
  ...
  it("refuses to write an API key to disk when the keychain is broken", async () => {
    hoisted.controls.broken = true;

    await expect(
      writeProfile({ url: "https://mb.example.com", apiKey: "key-agent" }),
    ).rejects.toThrowError(
      'Refusing to store a credential outside the OS keychain: the OS keychain is unavailable, and the "agent" profile store keeps secrets nowhere else. Nothing was written. Supply the credential in the environment instead (MB_URL/MB_API_KEY).',
    );
    expect(existsSync(profilesFilePath())).toBe(false);
  });

  it("refuses to write an OAuth token to disk when the keychain is switched off", async () => {
    process.env["MB_CLI_DISABLE_KEYRING"] = "1";

    await expect(writeOAuthProfile("https://mb.example.com", OAUTH)).rejects.toThrowError(
      'Refusing to store a credential outside the OS keychain: the OS keychain is disabled via MB_CLI_DISABLE_KEYRING, and the "agent" profile store keeps secrets nowhere else. Nothing was written. Supply the credential in the environment instead (MB_URL/MB_API_KEY).',
    );
    expect(existsSync(profilesFilePath())).toBe(false);
  });
});
