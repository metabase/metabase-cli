import { TestRunInputCompact } from "@metabase/client/domain/transform-test-run";

import { renderList } from "../../output/render";
import { listEnvelopeSchema } from "../../output/types";
import { testRunInputView } from "../../output/views/transform-test-run";
import { windowList } from "../../output/window";
import { connectionFlags, listFlags, outputFlags, profileFlag } from "../flags";
import { parseId, parseIdList } from "../parse-id";
import { defineMetabaseCommand } from "../runtime";

import { parseTargetType, targetLabels, targetTypeFlag } from "./subgraph";
...
export default defineMetabaseCommand({
  ...
  async run({ args, ctx, getClient }) {
    ...
    const { data } = await client.transformTest.inputs(targetType, target, { sources });
    renderList(windowList(data, ctx.range), testRunInputView, ctx);
  },
});
