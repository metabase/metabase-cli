...
describe("parseOptionalInteger", () => {
  ...
  it("rejects an empty string rather than reading a collapsed shell expansion as absent", () => {
    expect(() => parseOptionalInteger("", { name: "--port", min: 1 })).toThrow(
      new ConfigError('invalid --port: "" (expected integer)'),
    );
  });
