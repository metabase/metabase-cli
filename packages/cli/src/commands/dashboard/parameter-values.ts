...
export default defineMetabaseCommand({
  ...
  async run({ args, ctx, getClient }) {
    ...
    const search = args.query === undefined || args.query === "" ? null : args.query;
    const result =
      search === null
        ? await client.dashboard.parameterValues(dashboardId, parameterId)
        : await client.dashboard.searchParameterValues(dashboardId, parameterId, search);
