...
export default defineMetabaseCommand({
  meta: {
    name: "list",
    description:
      "List CLI-bundled skills — always consult the matching skill before acting on a task; they are the source of truth for every workflow. Skills the profile's server cannot use are left out; --all lists them too.",
  },
  requires: null,
  args: { ...outputFlags, ...listFlags, ...profileFlag, ...unavailableSkillsFlag },
  outputSchema: SkillListEnvelope,
  examples: ["mb skills list", "mb skills list --json", "mb skills list --all"],
  async run({ args, ctx }) {
    const profileName = resolveProfileName(args.profile);
    const profile = args.all === true ? null : await readCachedServerProfile(profileName);
