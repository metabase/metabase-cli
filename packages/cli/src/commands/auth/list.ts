...
import { type ListEnvelope, listEnvelopeSchema } from "../../output/types";
...
type AuthProfileStatusValue = z.infer<typeof AuthProfileStatus>;
...
export default defineMetabaseCommand({
  ...
  async run({ ctx }) {
    // Window the stored records before probing: each probe is a round trip, so `--limit 1` over
    // ten profiles should cost one, and the warnings below should name only rows the user saw.
    const window = windowList(await listProfileRecords(), ctx.range);
    ...
    for (const record of window.data) {
      items.push(await persistAndProject(await verifyOne(record)));
    }
    const envelope: ListEnvelope<AuthProfileJson> = { ...window, data: items };
    renderList(envelope, authProfileView, ctx);
...
async function verifyOne(record: ProfileRecord): Promise<VerificationEntry> {
  ...
  const result = await verifyAndProbe(resolved.url, resolved.credential, {
    refresh: createCredentialRefresher(record.name),
