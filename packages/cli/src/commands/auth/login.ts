...
// Which of the two reads `verifyAndProbe` runs came back bad, in the user's terms.
const VERIFY_STEP_LABEL: Readonly<Record<VerifyWhich, string>> = Object.freeze({
  user: "current user",
...
async function chooseLoginMethod(
  ...
  const method = await promptSelect<LoginMethod>({
  ...
  // Browser login needs a client id: one the user pre-registered and passed, or one the server
  // mints on demand. A server with dynamic registration disabled offers neither, and the client
  // raises the same refusal against its own `clientId` option — a spelling no CLI user can type.
  if (method === "oauth" && metadata.registration_endpoint === undefined) {
    throw new ConfigError(
...
async function completeLogin(
  ...
  const result = await verifyAndProbe(url, credential, { signal: interruptSignal });
  if (!result.ok) {
...
async function revokeUnsavedOAuthCredential(url: string, credential: Credential): Promise<void> {
  ...
  try {
    const revoked = await revokeOAuthCredential(url, credential, USER_AGENT);
    if (!revoked) {
      warn(
...
function formatVerifyFailureMessage(profileName: string, failure: VerifyFailure): string {
  const label = VERIFY_STEP_LABEL[failure.which];
  const which = failure.endpoint === undefined ? label : `${label}, ${failure.endpoint}`;
