...
import type { OAuthServerMetadata } from "@metabase/client/http/oauth";
...
describe("auth logout command", () => {
  ...
  it("names the CLI and its version on every OAuth request it issues", async () => {
    await writeOAuthProfile("https://m.example.com", OAUTH_CRED);
    const calls = installFetch([
      jsonResponse(METADATA),
      new Response("", { status: 200 }),
    ...
    expect(calls.map((call) => call.headers["user-agent"])).toEqual([
      USER_AGENT,
      USER_AGENT,
      USER_AGENT,
    ]);
