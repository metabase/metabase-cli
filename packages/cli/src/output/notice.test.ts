...
describe("listTruncationNotice", () => {
  ...
  it("names the offset to resume from when the cut left one", () => {
    expect(listTruncationNotice(2048, 12)).toBe(
      "… cut at 2048 bytes; continue with --offset 12, narrow the selection or raise --max-bytes",
    );
  });

  it("replaces the generic remedy with the command-specific hint when one is supplied", () => {
    expect(
      listTruncationNotice(2048, null, "request one skill at a time with `mb skills get`"),
    ).toBe("… cut at 2048 bytes; request one skill at a time with `mb skills get`");
  });
});
