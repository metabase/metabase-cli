...
const FINANCE_COLLECTION_NAME = "Finance";
const MONTHLY_REVENUE_CARD_NAME = "Monthly revenue";
const MONTHLY_REVENUE_SQL =
  "SELECT date_trunc('month', created_at) AS month, SUM(total_cents) AS gross_cents " +
  "FROM orders GROUP BY 1 ORDER BY 1";
const FINANCE_DASHBOARD_NAME = "Finance Overview";
const FINANCE_DASHBOARD_DESCRIPTION = "E2E seeded dashboard tracking revenue by month.";
...
async function seedContent(client: Client, libraryEnabled: boolean): Promise<SeededIds> {
  ...
  const orders = await createDashboardWithCard(client, {
  ...
  const financeCollectionId = await createEntityId(client, "/api/collection", {
    name: FINANCE_COLLECTION_NAME,
    color: "#88BF4D",
    parent_id: null,
  });

  const monthlyRevenueCardId = await createEntityId(client, "/api/card", {
    name: MONTHLY_REVENUE_CARD_NAME,
    display: "line",
    visualization_settings: {},
    collection_id: financeCollectionId,
    dataset_query: {
      type: "native",
      database: warehouseDbId,
      native: { query: MONTHLY_REVENUE_SQL },
    },
  });

  const finance = await createDashboardWithCard(client, {
    name: FINANCE_DASHBOARD_NAME,
    description: FINANCE_DASHBOARD_DESCRIPTION,
    collectionId: financeCollectionId,
    cardId: monthlyRevenueCardId,
  });
  ...
  return {
    ...
    defaultCollectionId,
    ordersCardId,
    ordersDashboardId: orders.dashboardId,
    ordersDashcardId: orders.dashcardId,
    financeCollectionId,
    monthlyRevenueCardId,
    financeDashboardId: finance.dashboardId,
    financeDashcardId: finance.dashcardId,
    tables,
    fields,
...
interface SeededDashboardInput {
  name: string;
  description: string;
  collectionId: number;
  cardId: number;
...
async function createDashboardWithCard(
  ...
  if (dashcardId === undefined) {
    throw new Error(`expected dashboard "${input.name}" to have a dashcard after PUT`);
  }
