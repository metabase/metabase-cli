...
const MONTHLY_REVENUE_COMPACT = {
  id: SEEDED.monthlyRevenueCardId,
...
describe("search e2e", () => {
  ...
  it("--models card narrows the result to the cards-only set", async () => {
    ...
    // Relevance decides the order of an unqueried search; the cards-only set is what this asserts.
    const byName = envelope.data.toSorted((a, b) => a.name.localeCompare(b.name));
    expect({ ...envelope, data: byName }).toEqual({
      data: [MONTHLY_REVENUE_COMPACT, ORDERS_BY_STATUS_COMPACT],
      returned: 2,
      total: 2,
      limit: 20,
    });
