...
- Full calendar date dimensions are rare. Never demand one; test date
  _semantics_ — ranges, orderings, volatile derivations.
...
Chain: `orders` + `customers` → _enriched_orders_ (order grain; LEFT JOIN
attaches `customer_name`, `tier`) → _fulfillment_by_tier_ (tier grain:
...
Derived, per catalog: grain uniqueness on both transforms _and the card_;
