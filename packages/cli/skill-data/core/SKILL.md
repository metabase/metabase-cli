...
1. **Understand the schema.** Read `.metadata/databases/` (`metabase-database-metadata`); never `table_metadata.json`. Profile with `mb table get <id> --include fields`, `mb field summary <id>` and `mb query`.
...
| Database metadata              | `db sync-schema`, `db rescan-values`, `metadata extract`                                                                                                                                                                                                                                                                                                                                  |
...
- **db traversal.** `.metadata/databases/` answers most schema questions without a call. On the instance: `db get <id> --include tables` for the table map, then `table fields <id>` per table; `--include tables.fields` only on small databases; hundreds of tables, go by schema (`db schemas`, `db schema-tables <db-id> <schema>`) or `search <term> --models table --db-id <id>`. `sync-schema --wait` blocks until the sync completes; `rescan-values` queues and returns.
...
- **`metabase-database-metadata`**: reading `.metadata/databases/`.
