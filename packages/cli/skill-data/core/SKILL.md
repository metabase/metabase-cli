...
## Query results, extracts, and failures

`mb query` runs an ad-hoc body (`--print-schema` → `--dry-run` → run; bodies are `mbql`'s), `mb card query <id>` a saved card. Both answer with `{data: {rows, cols}, …}`.

- **The row ceiling is the server's, not the CLI's.** `mb query` returns at most **2000 rows** for a bare-row query and **10 000** for an aggregated one (admin settings), and `--max-bytes 0` lifts the CLI's output cap, not that ceiling. For a full extract, save the query as a card, stream it, archive the card: `mb card create --file extract.json --json | jq -r '.id'`, then `mb card query <id> --export-format csv > extract.csv`, then `mb card archive <id>`.
- **A failure writes to stderr, so a pipe sees an empty result.** Under `--json` the CLI puts `{ok:false,error:{category,message,exitCode}}` on **stderr** and leaves stdout empty, so `mb query … --json | jq '.data.rows[]'` on a failed query prints nothing and reads like "no rows". A query that _runs_ and fails (bad SQL) is different: the server answers 200, the CLI prints `{status:"failed", error:…}` on **stdout** and exits **0**. Check both: `out=$(mb query --file q.json --json) || exit 1; jq -e '.status == "completed"' <<<"$out"`.
- **DDL has no path through `mb query`.** `CREATE` / `DROP` / `ALTER` go to the same read-oriented endpoint as a `SELECT`, and the driver answers that the statement did not produce a result set — an error that says nothing about whether the warehouse applied it. Confirm with a follow-up `SELECT`; author a `transform` for anything that writes.
...
- **card.** `dataset_query` is the **flat** `mbql/query` value, not a legacy `{type:"query",query:…}` envelope (→ `mbql`). `archive` is the only delete; unarchive with `update --body '{"archived":false}'`. `visualization_settings` keys are scoped by `display` and aren't pre-flighted — see `visualization`.
- **dashboard.** Dashcards round-trip through `PUT /api/dashboard/:id`: `update-dashcard <dash-id> <dashcard-id>` patches one; `update --body '{"dashcards":[…]}'` replaces the whole set (omitted ids are deleted; negative ids for new cards). Every dashcard carries `card_id` (`null` plus `visualization_settings.virtual_card` for text/heading/link cards); `create` takes the same array. Bad `card_id` refs exit **2** with `{ok:false,errors:[…]}`. `dashboard get <id>` hydrates dashcards/tabs; `list` omits them. **The grid is 24 columns wide** (`size_x: 24` is full width; keep `col + size_x ≤ 24`, no overlaps — the server stores collisions as sent). Layout patterns and per-chart sizes → the `dashboard` skill; load it before composing any `dashcards` array.
...
- **segment / measure.** `update` and `archive` require a non-blank `revision_message` (audit-logged); the CLI does not synthesize it on `update`. `archive` defaults to `"Archived via mb CLI"` — override with `--revision-message`. `definition` is a flat MBQL clause: segment = a filter, measure = exactly one aggregation. (→ `semantic-layer`, `mbql`.)
...
- **transform.** Iterate with `transform update <id>`, never `delete` + `create` (keeps the row, `entity_id`, materialized table, and YAML filename). `transform run` needs `--wait` (or `--sync`, which also waits for the output table to register and returns `target_table_id`) or you get only `{run_id, final:null}`. A `table-incremental` target keeps its watermark across `delete-table`, so the next run rebuilds with only newer rows. (→ `transform`.)
...
- **library.** EE-only (`library` premium feature, v59+). A curated subtree (`library-data` "Data" + `library-metrics` "Metrics" under a `library` root): published tables appear first in data pickers and rank up in search; Library metrics are prioritized in nav, search, and the query builder. `library get` shows the Data/Metrics collection ids, `library create` provisions it (idempotent), and `library publish --table-ids/--db-ids/--schemas` resolves the Data collection itself — each `--schemas` entry is `<db-id>:<schema>` (e.g. `1:public`), not a bare name. Publishing needs **admin or data-analyst** and cascades to upstream FK dependencies; `table get`/`table list` carry `is_published`. (→ `semantic-layer`.)
- **uuid.** `mb uuid --count <n>` mints the `lib/uuid` values MBQL clauses need (→ `mbql`).
...
- **`semantic-layer`** — models, metrics, measures, segments, and Library publishing: which verb builds each, the single-table reach rule, where a definition surfaces in the product, and how to verify it landed.

If a task spans more than one, load each. `mb skills list` enumerates everything on the installed version.

Higher-level data-engineering workflows — raw data to clean tables, the semantic layer, dashboards — live in the external `rde` skill, which drives this CLI: `npx skills add metabase/agent-skills --skill rde -a claude-code`.
