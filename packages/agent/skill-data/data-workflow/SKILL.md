---
name: data-workflow
description: Guided, end-to-end data work — investigate a raw database, build clean analysis-ready tables, define reusable segments/measures/metrics, answer questions, and build dashboards. Detects where the data is, holds the shared conventions for collaborating with a human on data work, and carries the deep per-stage method in references. Use when the user states a data goal rather than a single action — "make sense of my data", "build a data model", "go from raw data to a dashboard", "be my data analyst", "set up analytics for X", "define active customers / MRR officially", "make this reusable", "who registered / what did people say".
---

# Data workflow

The front door for turning a raw database into clean tables, reusable definitions, dashboards, and answers. You're the router and the conventions, not the worker: work out where the user is, set up shared context once, then load the right stage and let it drive.
...
The first three methods are references in this skill — read **only the one the current stage needs**. "Build dashboards" lives in two standalone skills — `visualization` for authoring each chart, `dashboard` for laying cards out on the grid and wiring filters and interactivity. Load `dashboard` before composing any dashboard, even a plain no-filter layout: the grid geometry lives there.

Query and body grammars come from the reference skills, not from here: `mbql` (query and definition bodies), `native-sql` (raw SQL, template tags), `transform` (transform sources, targets and runs), `metadata` (field and table curation), `library` (publishing tables). This skill owns the _judgment_ — which tables, which definitions, what to keep — and the conventions below.

## Setup — do this once, up front

**How hands-on they want to be — the autonomy slider.** Ask once, plainly:
...
The instance you're acting on, its URL, and the user you're acting as are already in your context. That URL is the base the browser links in the references are built from; you can't open a browser, so paste a link and let the user click it.
...
**Capability limits — know what you can't do.** Your tools author and query content, but they aren't the whole Metabase product. When the user asks for something outside their reach — alerts/subscriptions, applying a segment as a dashboard filter, scheduled emails, permissions UI — say so plainly and offer the nearest thing you _can_ do. Don't attempt it, hit a server error, and surface raw SQL or a stack trace.
...
2. **Surface and diagnose in plain terms.** Name what was denied and the likely reason. The usual three: _right table, wrong login_ — it exists, but the user you act as isn't granted it (common on staging — a config thing, not a data problem); _right name, wrong copy_ — a readable table of a similar name lives in another schema/database; _name slightly off_ — what they called it isn't the real table name. E.g. "I can't read `analytics.account` — this login doesn't have access. That's usually a staging-permissions thing, not a problem with your data."
3. **Offer to search — don't auto-crawl.** Ask first: "Want me to look for a table with a similar name I _can_ read?" Only on yes, run `search` (`{query: "account", type: ["table"]}`) or `browse_data` (`{action: "list_tables", database_id: <id>}`), and surface any match as a **confirm question**, never a substituted answer.
4. **Hand control back.** Don't propose or run a fix you can't reliably execute — no `GRANT`, no switching identity. Recovery is the user's call.

**Scratch files.** SQL and query bodies you're iterating on go in files under `./.scratch` in the working directory, never `/tmp`. Every tool that takes a body also takes a path: `execute_sql`'s `sql_file`, `execute_query`'s `query_file`, `transform_write`'s `source_file` / `native.sql_file`, `question_write`'s `query_file` / `native.sql_file`, `dashboard_write`'s `layout_file`. Edit the file, re-run it, then hand the **same file** to the write tool — that is how what you saved is byte-identical to what you tested. Editing something already saved starts the same way from the other end: `question_write`, `dashboard_write`, `transform_write` and `document_write` take `{method: "pull", id}`, which writes the saved body to a file for the same edit → test → update loop.
...
**Data not in a database yet?** A local CSV gets in via `upload_csv` (`{action: "create", file: "sales.csv"}` — creates a table plus a model over it, and returns both ids); treat the result as a starting table and clean it with a transform if needed. Uploads must be admin-enabled — check with `instance_settings` (`{action: "get", key: "uploads-settings"}`; `db_id: null` ⇒ not configured). Only a table Metabase itself created from an upload can be appended to or replaced (`{action: "append", file: …, table_id: …}`).
...
**When you do crawl,** walk `browse_data` narrowest-first, never a whole-warehouse rollup: `{action: "list_databases"}` → `{action: "list_schemas", database_id}` → `{action: "list_tables", database_id, schema}` → `{action: "get_fields", table_ids: [...]}` (batched — one call for several tables, and each field carries its `semantic_type` and `fk_target_field_id`). Existing curated tables: `{action: "list_models", database_id}`. Have a _name_ rather than a tree to walk? `search` (`{query: "orders", type: ["table"]}`). Need to know what's in a column? There is no field-profiling tool — profile it with a query: `execute_sql` a `GROUP BY` (see the reference files for the exact patterns). If a database looks freshly connected or an expected table is missing, offer `metadata_write` (`{action: "sync_schema", database_id, wait: true}`) before concluding it doesn't exist.
