...
> Part of the **`data-workflow`** skill — the "define reusable metrics" stage. It assumes that skill's **Shared Contract** (how to communicate, PII, autonomy, permission-denied) and final-recap rule. Body grammars: the `mbql` skill (every definition body), `library` (publishing tables).
...
- **Segment** — a saved filter on a table. A reusable row-selector: "Active customers", "orders over $100", "EU shipments". People pick it from the **Filter** block in the query builder instead of re-typing the conditions. Built with `segment_write`. (Docs: <https://www.metabase.com/docs/latest/data-studio/segments>.)
- **Measure** — a saved aggregation on a table. A reusable calculation: "Net Promoter Score", "average order value". People pick it from the **Summarize** block instead of re-writing the formula. Built with `measure_write`. (Docs: <https://www.metabase.com/docs/latest/data-studio/measures>.)
- **Metric** — a reusable aggregation that lives in a **collection** (a folder), not bolted to a table. "Monthly recurring revenue", "weekly active users". It's the org's official definition of an important number, can be saved into the **Library**, and can carry a default time dimension for charting. Built with `question_write` and `card_type: "metric"`. (Docs: <https://www.metabase.com/docs/latest/data-modeling/metrics>.)
...
This stage runs **after** the analysis-ready tables exist (make the table wider first — the **build-clean-tables** stage, `references/building-clean-tables.md`). Segments and measures only reach one table (hard rule 4 below), so a semantic layer on raw, normalized tables is nearly useless: a real answer rarely lives in a single raw table. **Wide clean tables first, segments/measures/metrics second.**

Read the `mbql` skill before you write any definition body — a segment's `definition`, a measure's `definition`, and a metric's `query` are all MBQL 5, and their exact shapes aren't guessable.
...
- **Where the metric lives.** Metrics sit in a collection (folder) — `question_write`'s `collection_id`. Lean: put the org's blessed ones in the shared **Library** so they surface prominently; keep experimental ones in a working collection until trusted.
- **Publish the official tables to the Library.** The clean, analysis-ready tables your definitions sit on are the org's official starting points — the **Library** is how you mark them as such. Tables published to the Library's **Data** section appear _first_ when anyone picks a data source, nudging people toward your curated tables instead of raw warehouse ones. Lean: publish the wide clean tables you built the semantic layer on; hold back raw or half-built ones. Surface which tables you'd publish and confirm — publishing carries a table's upstream sources with it, so the set that lands is bigger than the set you name. (Library is a Pro/Enterprise feature; only admins and data analysts can publish.)
- **Default time dimension for a metric.** A monthly default makes it chart nicely on a dashboard, but doesn't lock anyone out of other groupings. It's the metric query's one time grouping — a metric takes exactly one aggregation and at most one temporal breakout. Lean: set a sensible default (usually month) for anything headline; leave it off for raw counts that aren't inherently time-series.
- **How strict a segment is.** "Active" = last 30 vs 90 days is a real business call with no right answer from the data alone. Lean: surface the few reasonable thresholds with how many rows each catches — one `execute_sql` count per threshold — and let the user pick.
...
Don't narrate. One "Let me see what's here and how people are already slicing it" is plenty. Keep it cheap — `browse_data`'s batched `get_fields`, `LIMIT`/`GROUP BY` samples, never whole-warehouse rollups.

1. **Confirm the analysis-ready tables exist.** `browse_data` (`{action: "list_tables", database_id, schema}`) — find the wide, clean ones (a transform's output). If the user is pointing you at raw normalized tables, say so plainly and suggest building the clean table first — don't build a hobbled semantic layer on raw data.
2. **Find the questions people keep asking.** `search` the saved content (`{query: "revenue", type: ["card", "dataset", "metric", "dashboard"]}`), then `get_content` (`{items: [{type: "question", id: 42}], include: ["definition"]}`) to read the actual queries. Hunt for repeated filters and repeated calculations — the same "status = active" written eleven times, five hand-rolled versions of revenue. Those repeats _are_ the semantic layer waiting to be named. This is the highest-signal input; mine it before proposing anything.
3. **Learn the real meanings.** For every candidate segment ("active", "churned", "high-value"), find what the words map to in actual values — `SELECT status, count(*) FROM customers GROUP BY 1 ORDER BY 2 DESC` for a status column, a min/max/percentile read for an amount column. Pin every definition to real data (hard rule 1).
...
5. **Check reach before promising.** For each candidate, confirm it can actually live where it needs to: a single-table segment/measure must sit on the table people will build questions on — the `table_id` you'd pass to `segment_write` / `measure_write` is exactly that table, and if the facts it needs aren't all on it, the definition can't exist. A multi-table answer needs a wider table first (hard rule 4). Catch this now, not after building something that won't appear.
...
Build each agreed definition. Every body is MBQL 5 — the `mbql` skill has the grammar:

- **Segment** → `segment_write` `{method: "create", name, table_id, definition}`. The `definition` is a single-stage query holding only `filters`.
- **Measure** → `measure_write` `{method: "create", name, table_id, definition}`. The `definition` is a single-stage query holding **exactly one** `aggregation`.
- **Metric** → `question_write` `{method: "create", card_type: "metric", name, query, collection_id}` — it lives in a **collection**, and its query carries the one aggregation plus at most one time grouping (that's the default time dimension). Put org-blessed ones in the Library's Metrics collection (`library` `{action: "get"}` reports its id).
- **Publish the official tables** → `library` `{action: "publish", table_ids: [...]}` moves the clean tables your definitions sit on into the Library's **Data** section, so people start from your curated set, not raw warehouse tables.

Revising an existing definition is an update, never a delete-and-recreate: `segment_write` / `measure_write` with `{method: "update", id, revision_message: "…"}`. The `revision_message` is the audit note recorded in the revision history, and the API rejects an update without it — say what changed and why.

Then **verify what the user can't see**, before you hand back:

- **Each segment narrows the rows you expect.** Run the definition's own body as a count with `execute_query` — same `source-table`, same `filters`, plus a `count` aggregation — and compare against the table's unfiltered `count(*)`. Does "active customers" really return ~2,400 of 6,000?
- **Each measure and metric returns a sane number**, not null or an error. Run its body the same way.
- **Each definition sits where the user will look for it.** `get_content` (`{items: [{type: "segment", id: 7}], include: ["definition"]}`) echoes back the `table_id` and the body that was saved — confirm the table is the one people build questions on, and the filters are the ones you agreed. A definition attached to the wrong table (or to one that would need a join) is the classic silent failure: it never appears in the Filter or Summarize block, and nobody finds out until they go looking. Catch it here.
...
- **Don't** create a segment from the phrase alone. **Find the real meaning first:** `search` the existing questions and read their definitions with `get_content` — three people filter on "last seen in the last 30 days", two on "subscription status = active". That's the ambiguity to resolve. Ask: "I see two takes on 'active' — seen in the last 30 days, or has a live subscription. Which do you mean?" (hard rule 1).
- They say "live subscription, and seen in the last 30 days." **Check reach:** both pieces of info must live on the one table people build questions on. If subscription status and last-seen sit on two different tables, a single segment can't span them (hard rule 4) — to the user: "those two facts live in different places right now, so I'll widen your Customers table to carry both first, then save the filter on it." Make the table wider first (the **build-clean-tables** stage, `references/building-clean-tables.md`), then the segment on the wide table.
- Build it with `segment_write` on the wide table. **Verify** the row count is plausible by running the same filters as a count. **Recap** plainly and stop: "Saved **Active users** — live subscription and seen in the last 30 days — as a segment on your Customers table; it's in the Filter block there. Have a look before you build on it."
