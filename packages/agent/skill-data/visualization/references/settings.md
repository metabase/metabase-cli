...
All column-naming keys (`graph.dimensions`, `pie.dimension`, `table.columns[].name`, `map.latitude_column`, …) take **output column-name strings** — the names the query produces. Every key and value below is identical in the API form (`question_write`'s `visualization_settings`, a dashcard's `visualization_settings` in `dashboard_write`'s layout) and the portable git-sync form, with two exceptions: `column_settings` `["ref", …]` keys and click-behavior dimension targets carry a numeric field id in the API form and a name-path in the portable form. In JSON, `column_settings` keys are escaped strings: `"[\"name\",\"TOTAL\"]"`.
...
| `pie.rows`               | object[]           | per-slice order, label, color, visibility (below) | palette order          |

Slice colors are settable, but only per slice and only as concrete colors — there is no palette or color-scheme key. Each `pie.rows` entry is `{ key, name, color, defaultColor, enabled }`: `key` is the dimension value the slice comes from, `color` a hex string, and `defaultColor: false` is what makes your `color` win over the auto-assigned one. Omit `pie.rows` and every slice takes a palette color in metric order. (A legacy `pie.colors` map — dimension value → hex — is still read on cards that carry it; `pie.rows` wins where both exist.)

```yaml
pie.rows:
  - { key: Gadget, name: Gadget, color: "#509EE3", defaultColor: false, enabled: true }
  - { key: Widget, name: Widget, color: "#88BF4D", defaultColor: false, enabled: true }
```
...
A text, heading, link or iframe tile is a dashcard with no card behind it. In `dashboard_write`'s layout a dashcard declares one as its content source — `text`, `heading`, `link` (`{url}` or `{entity: {type, id}}`), or `iframe` — and the stored `visualization_settings` below are assembled for it. This is the shape you read back, and the shape any extra keys (`dashcard.visualization_settings`) sit alongside:

| `virtual_card.display` | Extra keys                                                                                                     |
| ---------------------- | -------------------------------------------------------------------------------------------------------------- |
| `heading`              | `text` (the heading string)                                                                                    |
| `text`                 | `text` (markdown; `{{param}}` placeholders wired by a dashcard `parameter_mappings` entry whose `target_tag` names the placeholder) |
...
Click behavior is a **dashboard** feature — author it in a **dashcard's** `visualization_settings` inside `dashboard_write`'s layout document, either whole-card or per column under `column_settings[<key>].click_behavior`, **not** in a saved card's own `visualization_settings`. There is no query-builder UI for it, and the interactive types need the dashboard around them: `crossfilter` reads the dashboard's parameters, and links to a `question`/`dashboard` resolve against the dashboard's target entities — both do nothing on a standalone card. (A bare URL link is the only type not strictly gated by dashboard context, but it's still only authored on dashcards.)
