...
/**
 * Normalize a family (single name or comma-separated list) into a safe CSS
 * font-family list, or null when the input is effectively empty. An unquoted
 * name that is not a CSS ident ("3270 Nerd Font") makes the whole canvas font
 * shorthand invalid and the assignment silently no-ops.
 */
...
/**
 * Whether a family renders every character on the same advance. A cell grid
 * requires it: a proportional face draws its text narrower than the lattice
 * the cursor and selection are placed on, which reads as ragged gaps and a
 * cursor stranded to the right of the text.
 *
