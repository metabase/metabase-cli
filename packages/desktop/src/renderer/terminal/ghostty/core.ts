...
/**
 * `GhosttyTerminalOption` and `GhosttyTerminalData` values from
 * `ghostty/vt/terminal.h` at the revision pinned in
 * `native/libghostty-vt/VERSION`; the other numbers passed to
 * `ghostty_terminal_set`/`_get` here are from the same enums.
 */
const TERMINAL_OPT = {
  colorForeground: 11,
  colorBackground: 12,
...
/**
 * The 16 ANSI colours in Ghostty's palette order: black, red, green, yellow,
 * blue, magenta, cyan, white, then the same eight bright.
 */
export type GhosttyAnsiPalette = readonly [
  GhosttyColor,
  GhosttyColor,
  GhosttyColor,
  GhosttyColor,
...
export interface GhosttyTheme {
  ...
  /** Replaces palette entries 0-15; 16-255 keep Ghostty's defaults. Omitted leaves all 256 alone. */
  readonly palette?: GhosttyAnsiPalette;
}
...
function channel(front: number, back: number): number {
  return Math.floor((front * 155 + back * 100) / 255);
}
...
function keyAction(action: "press" | "release", event: Pick<KeyboardEvent, "repeat">): number {
  if (action === "release") {
    return 0;
  }
...
/** Byte offsets of the GhosttyStyle flags inside the struct the module publishes. */
interface StyleFieldOffsets {
...
export class GhosttyTerminalCore {
  ...
  private styleFieldOffsets: StyleFieldOffsets | null = null;
  ...
  setTheme(theme: GhosttyTheme): void {
    ...
    for (const [option, value] of [
      [TERMINAL_OPT.colorForeground, theme.foreground],
      [TERMINAL_OPT.colorBackground, theme.background],
      [TERMINAL_OPT.colorCursor, theme.cursor],
    ] as const) {
  ...
  /**
   * Ghostty takes the palette as one array of 256 colours, and the theme names
   * only the 16 ANSI ones: read its own defaults back first (the default
   * palette, which no OSC 4 an application sent can have touched) and replace
   * the head, so the 6x6x6 cube and the greys stay the engine's rather than
   * being reproduced here.
   */
  private setPalette(palette: GhosttyAnsiPalette): void {
    const size = PALETTE_ENTRIES * 3;
    const buffer = this.runtime.alloc(size);
    if (
      this.runtime.call(
        "ghostty_terminal_get",
        this.terminal,
        TERMINAL_DATA.colorPaletteDefault,
        buffer,
      ) === GHOSTTY_SUCCESS
    ) {
      const bytes = this.runtime.bytes(buffer, size);
      palette.forEach((color, index) => bytes.set([color.r, color.g, color.b], index * 3));
      this.runtime.call("ghostty_terminal_set", this.terminal, TERMINAL_OPT.colorPalette, buffer);
    }
  ...
  scroll(deltaRows: number): void {
    ...
    const value = this.runtime.field("GhosttyTerminalScrollViewport", "value");
  ...
  encodeKey(event: KeyboardEvent, action: "press" | "release" = "press"): string {
    ...
    this.runtime.call("ghostty_key_event_set_action", this.keyEvent, keyAction(action, event));
  ...
  encodeMouse(input: GhosttyMouseInput): string {
    ...
    positionView.setFloat32(this.runtime.field("GhosttyMousePosition", "x").offset, input.x, true);
    positionView.setFloat32(this.runtime.field("GhosttyMousePosition", "y").offset, input.y, true);
  ...
  setSelection(anchor: GhosttyPointInput, end: GhosttyPointInput): void {
    ...
    try {
      ...
      const startField = this.runtime.field("GhosttySelection", "start");
      const endField = this.runtime.field("GhosttySelection", "end");
      this.runtime
  ...
  private readRow(
    ...
    const styleSize = this.runtime.layout("GhosttyStyle").size;
    const styleOffsets = this.styleOffsets();
    ...
    while (
      ...
      if (styleView.getUint8(styleOffsets.inverse) !== 0) {
        [foreground, background] = [background, foreground];
      }
      if (styleView.getUint8(styleOffsets.faint) !== 0) {
      ...
      cells.push({
        ...
        foreground,
        background,
        bold: styleView.getUint8(styleOffsets.bold) !== 0,
        italic: styleView.getUint8(styleOffsets.italic) !== 0,
        invisible: styleView.getUint8(styleOffsets.invisible) !== 0,
        strikethrough: styleView.getUint8(styleOffsets.strikethrough) !== 0,
        overline: styleView.getUint8(styleOffsets.overline) !== 0,
        underline: styleView.getInt32(styleOffsets.underline, true) !== 0,
        selected,
      });
    }
  ...
  /**
   * The GhosttyStyle offsets the row reader needs, resolved once: the layout
   * is the module's and never changes, and the loop runs per cell per frame.
   */
  private styleOffsets(): StyleFieldOffsets {
    this.styleFieldOffsets ??= {
      bold: this.runtime.field("GhosttyStyle", "bold").offset,
      italic: this.runtime.field("GhosttyStyle", "italic").offset,
      faint: this.runtime.field("GhosttyStyle", "faint").offset,
      inverse: this.runtime.field("GhosttyStyle", "inverse").offset,
      invisible: this.runtime.field("GhosttyStyle", "invisible").offset,
      strikethrough: this.runtime.field("GhosttyStyle", "strikethrough").offset,
      overline: this.runtime.field("GhosttyStyle", "overline").offset,
      underline: this.runtime.field("GhosttyStyle", "underline").offset,
    };
    return this.styleFieldOffsets;
  }
  ...
  private gridRef(col: number, row: number, tag: 1 | 2 = 1): number {
    ...
    const pointValue = this.runtime.field("GhosttyPoint", "value");
  ...
  private selectAt(
    ...
    try {
      ...
      const refField = this.runtime.field(optionsName, "ref");
      this.runtime
      ...
      if (result === GHOSTTY_SUCCESS) {
        const start = selection + this.runtime.field("GhosttySelection", "start").offset;
        const end = selection + this.runtime.field("GhosttySelection", "end").offset;
