...
import { cssFontFamilies, isMonospaceFamily } from "./fonts";
import symbolsFontUrl from "../../fonts/SymbolsNerdFontMono-Regular.woff2?url";
import { isSingleCodePoint } from "./keyCodes";
...
/**
 * Browser input, IME, selection, scrolling, sizing, links and cursor blinking
 * for one terminal.
 */
...
function uncheckedTerminalFontFamily(family?: string): string {
  const custom = family === undefined ? "" : (cssFontFamilies(family) ?? "");
...
/**
 * Whether the copy chord means copy and nothing else, so that with nothing
 * selected it is swallowed rather than sent to the shell — `⌘C` is not a key
 * a shell has ever received, and encoding it types a `c` at the prompt. A
 * plain `Ctrl+C` off macOS is the exception: it is also SIGINT, and with no
 * selection it belongs to the program that is running.
 */
export function isTerminalCopyOnlyShortcut(
  event: Pick<KeyboardEvent, "ctrlKey" | "key" | "metaKey" | "shiftKey">,
  platform = navigator.platform,
): boolean {
  ...
  return isMacPlatform(platform) || event.shiftKey || event.key.toLowerCase() === "insert";
}
...
/**
 * The `InputEvent` fields the surface reads. `addEventListener("input", …)`
 * hands over a bare `Event`, so they are read off it structurally rather than
 * assumed: a browser that fires a plain `Event` (or a test that synthesizes
 * one) reads as "no data, no composition" instead of throwing.
 */
function terminalInputEventFields(event: Event): {
  readonly data: string | null;
  readonly inputType: string;
  readonly isComposing: boolean;
} {
  return {
    data: "data" in event && typeof event.data === "string" ? event.data : null,
    inputType: "inputType" in event && typeof event.inputType === "string" ? event.inputType : "",
    isComposing: "isComposing" in event && event.isComposing === true,
  };
}
...
export function isTerminalAltGraphText(
  ...
  return event.getModifierState("AltGraph") && isSingleCodePoint(event.key);
}
...
function wheelDeltaScale(deltaMode: number, cellHeight: number, viewportRows: number): number {
  switch (deltaMode) {
    case 1: {
      return cellHeight;
    }
    case 2: {
      return viewportRows * cellHeight;
    }
    default: {
      return 1;
    }
...
export function terminalWheelDeltaRows(
  ...
  const pixels = event.deltaY * wheelDeltaScale(event.deltaMode, cellHeight, viewportRows);
...
export function terminalWheelArrowData(rows: number, applicationCursorKeys: boolean): string {
  ...
  const application = rows < 0 ? "\u001bOA" : "\u001bOB";
  const normal = rows < 0 ? "\u001b[A" : "\u001b[B";
  const sequence = applicationCursorKeys ? application : normal;
...
/** Click, double-click, triple-click, and back to a single click. */
function nextClickCount(count: number): number {
...
export function advanceTerminalSelectionClickSequence(
  ...
  return {
    count: repeats ? nextClickCount(previous.count) : 1,
...
/** One click selects cells, two a word, three the whole line. */
function selectionModeFor(clickCount: number): "cell" | "word" | "line" {
  if (clickCount >= 3) {
    return "line";
  }
  return clickCount === 2 ? "word" : "cell";
}

/** Dragging past the top or bottom edge scrolls the viewport that way. */
function autoscrollDirection(clientY: number, bounds: { top: number; bottom: number }): number {
  if (clientY < bounds.top) {
    return -1;
  }
  return clientY > bounds.bottom ? 1 : 0;
}
...
function anchorOf(range: ScreenRange, fromEnd: boolean): { x: number; y: number } {
  return fromEnd ? { ...range.end } : { ...range.start };
}
...
export interface GhosttyTerminalSurfaceOptions {
  ...
  /** The viewport moved under the viewer's own gesture (wheel, scrollbar, jump). */
...
export class GhosttyTerminalSurface {
  ...
  static async create(
    ...
    scrollbarThumb.className =
      "absolute inset-x-px top-0 rounded-chip bg-line-strong transition-colors group-hover:bg-ink-3 group-focus-visible:bg-ink-3";
  ...
  write(data: string): void {
    this.writeBytes(encoder.encode(data));
  }

  /**
   * The shell's output as it came off the PTY. The transport carries bytes
   * and a chunk boundary can fall inside a UTF-8 sequence or an escape, which
   * only the VT parser can carry across; decoding per chunk would corrupt it.
   */
  ...
  /** Clears the screen and the scrollback, as a stream that reopened is about to replay them. */
  reset(): void {
    this.resetAndWrite("");
  }
  ...
  fit(): boolean {
    ...
    if (grid.cols !== this.cols || grid.rows !== this.rows || !this.resizeNotified) {
      // A selection is the cells the viewer dragged over, and a reflow moves
      // the text under it. Ghostty keeps every selected row on screen, so a
      // selection that reaches the blank rows below the prompt makes a shrink
      // push the content into scrollback instead of trimming those rows — the
      // screen looks cleared. The selection no longer means what was dragged
      // either, so it goes first and the grid reflows as the viewer expects.
      if (this.core.selectionText().length > 0) {
        this.clearSelection();
      }
      this.cols = grid.cols;
      this.rows = grid.rows;
  ...
  private readonly onKeyDown = (event: KeyboardEvent) => {
    ...
    if (isTerminalCopyOnlyShortcut(event)) {
      // The copy chord with nothing selected: there is nothing to write, and
      // the shell must not receive it either.
      this.suppressedKeyCodes.add(event.code);
      event.preventDefault();
      return;
    }
  ...
  private readonly onInput = (event: Event) => {
    const inputEvent = terminalInputEventFields(event);
  ...
  private beginSelection(event: { clientX: number; clientY: number }, clickCount: number): void {
    ...
    this.selectionMode = selectionModeFor(clickCount);
    const range = this.selectionRangeAt(cell);
    if (range) {
  ...
  private readonly onPointerMove = (event: PointerEvent) => {
    ...
    const bounds = this.canvas.getBoundingClientRect();
    this.setSelectionAutoscroll(autoscrollDirection(event.clientY, bounds));
    const cell = this.cellAt(event.clientX, event.clientY);
  ...
  private extendSelectionTo(clientX: number, clientY: number): void {
    ...
    // A drag that crossed behind where it started swaps the two ends: the
    // base's far edge becomes the anchor and the selection runs backwards.
    const anchor = base === null ? anchorScreen : anchorOf(base, beforeBase);
    const end = range === null ? cellScreen : anchorOf(range.screen, !beforeBase);
  ...
  /** What the current selection mode makes of a cell: a line, a word, or nothing beyond the cell itself. */
  private selectionRangeAt(cell: { x: number; y: number }): GhosttySelectionRange | null {
    switch (this.selectionMode) {
      case "line": {
        return this.core.selectLine(cell.x, cell.y);
      }
      case "word": {
        return this.core.selectWord(cell.x, cell.y);
      }
      case "cell": {
        return null;
      }
  ...
  /** The cell before this one, following a soft wrap to the row above; null at the start of the viewport. */
  private cellBefore(cell: { x: number; y: number }): { x: number; y: number } | null {
    if (cell.x > 0) {
      return { x: cell.x - 1, y: cell.y };
    }
    if (cell.y > 0 && this.snapshot?.rowData[cell.y]?.isWrapContinuation === true) {
      return { x: this.cols - 1, y: cell.y - 1 };
    }
    return null;
  }

  /** The cell after this one, following a soft wrap to the row below; null at the end of the viewport. */
  private cellAfter(cell: { x: number; y: number }): { x: number; y: number } | null {
    if (cell.x + 1 < this.cols) {
      return { x: cell.x + 1, y: cell.y };
    }
    if (cell.y + 1 < this.rows && this.snapshot?.rowData[cell.y]?.wrapsToNext === true) {
  ...
  private readonly onContextMenu = (event: MouseEvent) => {
    if (shouldReportTerminalMouse(this.core.isMouseTracking(), event)) {
      event.preventDefault();
      // The application claimed this click. Stopping it here is what keeps a
      // host menu bound to the event from opening on top of a program that is tracking the mouse.
      event.stopPropagation();
      return;
    }
  ...
  private readonly onScrollbarPointerMove = (event: PointerEvent) => {
    if (event.pointerId !== this.scrollbarPointerId || this.scrollbarState === null) {
      return;
    }
    event.preventDefault();
  ...
  private renderFrame(): void {
    ...
    // A terminal whose tab is not the active one keeps its host element (and
    // its scrollback) but has no size to draw into. Ghostty keeps parsing; the
  ...
  private linkAt(clientX: number, clientY: number): TerminalLinkWithRange | null {
    ...
    if (explicitHyperlink) {
      ...
      const end = { ...cell };
      while (true) {
        const previous = this.cellBefore(start);
        if (!previous || this.core.hyperlinkAt(previous.x, previous.y) !== explicitHyperlink) {
          break;
        }
        start.x = previous.x;
        start.y = previous.y;
      ...
      while (true) {
        const next = this.cellAfter(end);
        if (!next || this.core.hyperlinkAt(next.x, next.y) !== explicitHyperlink) {
          break;
        }
        end.x = next.x;
        end.y = next.y;
