...
describe("pollUntil", () => {
  ...
  it("rethrows the caller signal's reason when it is already aborted, never a timeout", async () => {
  ...
  it("rethrows the caller signal's reason when it aborts between polls", async () => {
    const controller = new AbortController();
    const reason = new AbortError("interrupted");
