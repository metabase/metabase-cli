...
describe("toMetabaseError", () => {
  ...
  it("maps a ZodError to a ConfigError", () => {
...
describe("MetabaseError contract", () => {
  it("AbortError exposes the documented category", () => {
  ...
  it("ConfigError exposes the documented category", () => {
    const error = new ConfigError("missing TTY");
    expect(error.category).toBe("config");
    expect(error.userMessage).toBe("missing TTY");
  });

  it("InternalError separates a contract violation from a user config mistake", () => {
    const error = new InternalError("pageSize must be at least 1, got 0");
    expect(error.category).toBe("internal");
    expect(error.developerDetail).toBeNull();
    expect(error.userMessage).toBe("pageSize must be at least 1, got 0");
  });
});
...
function buildResponseShapeError(
  ...
  return ResponseShapeError.fromZodIssues({
    kind: "zod",
    method: "GET",
