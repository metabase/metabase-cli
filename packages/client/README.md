...
A server newer than the window, or a head build whose tag does not parse, is placed one past the newest known major — the slot a rule written against head describes — with `skew` set to `"newer-than-known"` or `"unknown"`; its additions pass through the loose schemas, and only a token feature can refuse it. The client never takes a version from the caller: a profile comes from a probe, so two consumers of one server cannot disagree about what it is.
