...
- Convention: `severity: error` = forbidden; `severity: warn` = tolerated,
  surfaced, and — where the oddity exists in real data — *designed to fire on
  every run*, with a fixture row guaranteeing it and the warehouse count in
...
runner fails on drift in *either* direction — a tolerance silently growing
...
fires *at the recorded rate*. A changed baseline is a finding to investigate,
