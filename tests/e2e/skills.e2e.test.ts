...
describe("skills e2e", () => {
  ...
  it("list returns the eight bundled non-hidden skills, sorted by name", async () => {
  ...
  it("get rejects an unknown skill name with exit 2 and a ConfigError message listing available names", async () => {
    ...
    expect(result.stderr).toContain(
      "unknown skill name(s): does-not-exist (available: core, document, git-sync, mbql, transform, visualization)",
    );
  });
