...
What those two patterns bound is a path literal and a transport call, not every client function that can open a socket. A call from `commands/` that reaches the network by another name is FAIL — the pattern rules cannot see it, so review is the only thing that can. The CLI's one `fetch` outside the client is `core/auth-broker.ts`, which talks to the app's credential broker.
...
- Credentials reach the CLI through `runCli`'s `env` (`MB_URL` + `MB_API_KEY` from `readBootstrap()`), never argv; a seeded probe goes through `tests/e2e/seed-probe.ts`.
