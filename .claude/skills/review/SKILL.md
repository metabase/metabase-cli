...
1. **`<Resource>`** — `z.object({ ... }).passthrough()` for the full API shape, with the type inferred via `z.infer`. `.passthrough()` is the default so Metabase API additions don't break the parse — tighten over time, don't preempt.
2. **`<Resource>Compact`** — a small projection (typically `<Resource>.pick({ ... })`) with its inferred type. This is the agent-facing contract.
...
Adding a resource is additive: new `src/domain/<r>.ts` + `tests/fixtures/<r>/sample.json` + a unit test that runs `<R>.parse(<fixture>)`. Editing existing commands to wire the schema is FAIL — commands import what they need.

A new or modified API resource without all three exports, or that omits `.passthrough()` on the full schema, or that hand-rolls `Array<Record<string, unknown>>` / inline `as { ... }` casts in commands instead of consuming `z.infer<typeof Schema>`, is a structural failure.
...
- New `src/domain/<r>.ts` ⇒ `tests/fixtures/<r>/sample.json` exists; a unit test invokes `<R>.parse(...)` against it.
...
5. **Domain contract** — every new or changed resource in `src/domain/` exports `<Resource>` (with `.passthrough()`), `<Resource>Compact`, and `<resource>View`; naming follows the convention; commands consume `z.infer<typeof Schema>` rather than `Array<Record<string, unknown>>` or inline `as { ... }` casts.
...
24. **Test schema reuse** — when an `src/` module exports a Zod schema describing CLI output (e.g. `LoginResult`, `AuthStatus`) or a Metabase resource (any `<Resource>` / `<Resource>Compact` in `src/domain/`), tests must import it. A test file that declares a `z.object({...})` mirroring the same fields as a known `src/` export is FAIL. Quote both the test declaration and the canonical export.
